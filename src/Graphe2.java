...
public class Graphe2 {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(chemin))) {
            ...
            for (int i = 0; i < n; i++) {
                if (lignes.get(i).length != n) {
                    throw new IOException("La matrice doit être carree.");
                }
    ...
    public boolean estConnexe() {
        ...
        // tous les sommets avec degre > 0 doivent être visités
        for (int i = 0; i < n; i++) {
    ...
    public void rendreEulerienEnDoublantDesAretes() {
        ...
        if (impairs.isEmpty()) {
            System.out.println("Tous les sommets sont de degre pair");
            return;
        }

        System.out.println("Sommets de degre impair avant correction : " + impairs);
        ...
        while (impairs.size() > 1) {
            ...
            if (pred[v] == -1) {
                throw new IllegalStateException("Pas de chemin entre " + u + " et " + v + " alors que le graphe devrait etre connexe.");
            }
        ...
        //System.out.println("Sommets de degre impair apres correction : " + impairsApres);
    }
    ...
    public List<Integer> circuitEulerien(int sommetDepart) {
        if (!estEulerien()) {
            throw new IllegalStateException("Le graphe n est pas eulerien.");
        }
